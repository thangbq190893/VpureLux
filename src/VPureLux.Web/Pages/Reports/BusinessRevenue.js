(function () {
    ...
    const summary = () => abp.ajax({ url: path + '?handler=Summary', type: 'GET', data: filters() }).then(x => { $('#RevenueSales').text(money(x.salesRevenue)); $('#RevenueService').text(money(x.serviceRevenue)); $('#RevenueTotal').text(money(x.totalRevenue)); $('#RevenuePaid').text(money(x.totalPaid)); $('#RevenueRemaining').text(money(x.totalRemaining)); });
