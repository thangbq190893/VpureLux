(function () {
    ...
    const table = $('#ServiceWorksTable').DataTable(abp.libs.datatables.normalizeConfiguration({ processing: true, serverSide: true, paging: true, searching: false, autoWidth: false, order: [], ajax: abp.libs.datatables.createAjax(input => abp.ajax({ url: abp.appPath + 'Service/Works?handler=List', type: 'GET', data: input })), columnDefs: [
        { data: null, orderable: false, rowAction: { items: [ { text: l('Edit'), action: data => modal.open({ id: (data.record || data).id }) }, { text: l('Deactivate'), visible: data => (data.record || data).isActive, action: data => { const row = data.record || data; abp.ajax({ url: abp.appPath + 'Service/Works?handler=SetActive&id=' + encodeURIComponent(row.id) + '&isActive=false', type: 'POST' }).then(() => table.ajax.reload(null, false)); } }, { text: l('Activate'), visible: data => !(data.record || data).isActive, action: data => { const row = data.record || data; abp.ajax({ url: abp.appPath + 'Service/Works?handler=SetActive&id=' + encodeURIComponent(row.id) + '&isActive=true', type: 'POST' }).then(() => table.ajax.reload(null, false)); } } ] } },
