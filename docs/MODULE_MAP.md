...
## Service (S-001 Foundation)
- **Purpose:** separate Work Catalog, not Sales products, with default-disabled runtime and recovery of historically applied Service schema.
- **Entities:** `ServiceWork` (Code, Name, nullable legacy Unit, DefaultPrice, nullable StandardCost, Status, Note, full ABP audit/concurrency). `ServiceOrder`, owned `ServiceOrderLine`, `ServicePayment` are passive persistence shells, without operational constructors or posting/state-transition commands.
- **Contracts/application:** `ServiceWorkContracts.cs`, `IServiceWorkAppService`, `ServiceWorkAppService`. Root/View permissions protect reads; ManageWorks additionally protects create/update. Every API checks `ServiceOptions.IsEnabled` (default false); no runtime config was enabled outside disposable test hosts.
- **EF/query:** `EfCoreServiceWorkRepository`, three configuration classes, DbSets and Work repository registration. SQL-side count/search/status/whitelisted sorting/Skip/Take plus Id tie-breaker. No lookup endpoint, fixed top-N load, historical string-max generator or business side effects.
- **UI:** `Pages/Service/Works*`, `WorkModal*`, `ServiceWorkForm`. One DataTable, ABP ModalManager create/edit, server authorization/antiforgery, nullable-cost display, vi-VN-safe form number binder scoped to Service only. Code stays immutable; optimistic concurrency prevents stale saves.
- **Permissions:** `Service`, `Service.View`, `Service.ManageWorks`; names match the historical catalog. Order/payment/report permission actions are not exposed early.
- **Migrations:** exact original pair `20260824113235_AddServiceModule` recovered; `20260906174229_AddServiceWorkCatalogFields` only adds nullable Unit/StandardCost on AppServiceWorks. No business DML/core alteration/duplicate Service CreateTable. All current Sales V1/CustomerCare/Warranty/Inventory mappings preserved. Neither migration was applied in S-001.
- **Tests/evidence:** Domain `ServiceWorkDomainTests`; Application `ServiceWorkContractTests`; EF `ServiceFoundationTests` (including application permission/feature execution and persisted historical line independence); Web `ServiceWorkWebTests`, `ServiceWorkUiSourceTests`. Offline SQL and browser/artifact evidence: `docs/S001_SERVICE_FOUNDATION.md`.
- **Boundary:** no ServiceOrder operator workflow, FIFO, payment posting, reminder integration, reports, deploy or push. Unknown legacy unit/cost stays unknown; editing catalog settings never refreshes existing document snapshots. S-002 is the next separately claimed task after S-001 acceptance gates.
