...
- **Entities:** `ServiceWork`; operational `ServiceOrder`; owned `ServiceOrderLine`; passive `ServicePayment`. Draft -> Confirmed -> InProgress -> Completed, with reason-required cancellation before completion and canonical exact replay.
- **Contracts/application:** `ServiceWorkContracts.cs`, `ServiceOrderContracts.cs`, `ServiceWorkAppService`, `ServiceOrderAppService`, `ServiceOrderOperationCoordinator`, `ServiceCompletionProcessor`. Orders expose CRUD/transitions/atomic Complete and paged lookups. `ICustomerCareServiceCompletion` owns care writes. No payment/report command is exposed.
...
- **Permissions:** `Service.{View,Create,Edit,Confirm,Cancel,Complete,ManageWorks}` plus root. Feature and permissions are checked server-side; Start reuses Confirm.
...
- **Boundary:** Confirm/Start remain free of stock/revenue/care/payment side effects. S-003 completion atomically posts FIFO, actual snapshots and care events/successors; payments remain S-004, reporting S-005 and external rehearsal/rollout S-006. Detailed tests, lock ordering and rollback evidence: `S003_SERVICE_COMPLETION.md`.
